    ...
    def ProcessIPWAction(self, widget, event, **args ):
        ...
        if event == ImagePlaneWidget.InteractionUpdateEvent:
            ...
            if action == ImagePlaneWidget.Cursoring:   
                cursor_data = widget.GetCursorData()
        ...
        for iz in range(dims[2]):   
            for iy in range(dims[1]):   
                for ix in range(dims[0]): 
                    val = 1.0 + math.sin( ix/w[0] ) * math.tanh(  iy/w[1] ) *  math.cosh( iz/w[2] ) 
                    image_data.SetScalarComponentFromDouble(ix,iy,iz,0,val)        
        return image_data
    
    
    def getLUT( self, n_col = 256 ):
        vtk_lut = vtk.vtkLookupTable()            
        vtk_lut.SetNumberOfColors( n_col )
        vtk_lut.SetTableRange( 0.0, 2.0 )
        vtk_lut.SetRampToSCurve()
        vtk_lut.Build()
        return vtk_lut
...
#    def getImageData( self, cdms_var, **args ):
...
#        
#        ndim = 3 
#        self._max_scalar_value = 65535.0
...
#        Relative_humidity = Relative_humidity(isobaric=(1000.0, 10.0),lon=(0.0, 359.0),time=('2011-5-1 0:0:0.0', '2011-5-1 18:0:0.0'),lat=(90.0, -90.0),squeeze=1,)
        ...
        origin = [0, 0, 0 ]
        extent =  [ 0, 100, 0, 100, 0, 50 ]
        spacing = [ 1, 1, 1 ]
        primaryInput = handler.getImageData( origin, extent, spacing )
        planeWidgetZ.SetInput( primaryInput )
